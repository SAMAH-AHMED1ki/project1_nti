...
| Field | Your answer |
|-------|-------------|
| **Full name** | Samah Ahmed Mahmoud Ahmed |
...
If R3 is placed before R1, the Garage lamp will incorrectly turn off instead of staying on. This happens because rules execute in sequential order (last write wins). If R3 runs first, it turns the lamp on due to overheating, but then R1 runs immediately after it and overwrites that decision based on occupancy (since the Garage is empty, occupied is no, so R1 turns the lamp off). Therefore, R3 is intentionally placed after R1 so its override action triumphs and keeps the overheating room's lamp lit.
